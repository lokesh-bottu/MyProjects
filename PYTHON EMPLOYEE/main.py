...
lblqua = Label(entries_frame, text="Qualification", font=("Calibri", 16), bg="#535c68", fg="white")
lblqua.grid(row=1, column=2, padx=10, pady=10, sticky="w")
txtqua = Entry(entries_frame, textvariable=qua, font=("Calibri", 16), width=30)
txtqua.grid(row=1, column=3, padx=10, pady=10, sticky="w")


lblarea = Label(entries_frame, text="Place", font=("Calibri", 16), bg="#535c68", fg="white")
lblarea.grid(row=1, column=4, padx=10, pady=10, sticky="w")
txtarea = Entry(entries_frame, textvariable=Place, font=("Calibri", 16), width=20)
txtarea.grid(row=1, column=5, padx=10, pady=10, sticky="w")
...
def add_employee():
    if txtName.get() == "" or txtqua.get() == "" or txtDoj.get() == "" or txtEmail.get() == "" or comboGender.get() == "" or txtContact.get() == "" or txtAddress.get(
            1.0, END) == "":
        messagebox.showerror("Erorr in Input", "Please Fill All the Details")
        return
    if txtqua.get()=="B.tech" or txtqua.get()=="M.tech" or txtqua.get()=="MS" :
        messagebox.showinfo("Congratulation","You are eligible for this Job")
        db.insert(txtName.get(),txtqua.get(), txtDoj.get() , txtEmail.get() ,comboGender.get(), txtContact.get(), txtAddress.get(1.0, END))
        messagebox.showinfo("Success", "Record Inserted")
        clearAll()
        dispalyAll()
    else:
        messagebox.showerror("Sorry","We are recruiting only Engineering Students")
...
def update_employee():
    if txtName.get() == "" or txtqua.get() == "" or txtDoj.get() == "" or txtEmail.get() == "" or comboGender.get() == "" or txtContact.get() == "" or txtAddress.get(
        ...
        messagebox.showerror("Erorr in Input", "Please Fill All the Details")
        return
    db.update(row[0],txtName.get(), txtqua.get(), txtDoj.get(), txtEmail.get(), comboGender.get(), txtContact.get(),
              txtAddress.get(
                  1.0, END))
...
tv.heading("3", text="Qualification")
tv.column("3", width=5)
